...
with open(path, 'r', newline='') as csvfile:
    ...
    data = dictify(reader, header_index=0, default_value=None, primary_key="# name",
                   value_columns=["star_radius", "semi_major_axis", "star_teff", "mass"])
...
for key, item in {key: similarity[key] for key in
                  ["GJ 667 C c", "Kepler-69 c", "LHS 1140 b", "TOI-700 d", "TRAPPIST-1 e", "GJ 273 b"]}.items():
    print(
        f"""Planet: {key}; \nTot. Change: {similarity[key]:.3f}; \nS.Sim: {similarity_s[key]:.3f}; Radius: {data[key][0]}; Axis: {
        data[key][1]}; Teff: {round(float(data[key][2]) / 5800, 3)}; \nMass: {round(320 * float(data[key][3]), 3)};""",
        end="\n\n\n")
